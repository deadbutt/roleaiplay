...
class RegisterCodeActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        // 注册按钮点击事件
        ctvRegister.setOnClickListener { verifyCode() }
    ...
    private fun verifyCode() {
        ...
        if (code.isEmpty()) {
            Toast.makeText(this@RegisterCodeActivity, "请输入验证码", Toast.LENGTH_SHORT).show()
            return
        }

        if (code.length != 6) {
            Toast.makeText(this@RegisterCodeActivity, "请输入6位验证码", Toast.LENGTH_SHORT).show()
            return
        }
        ...
        Thread {
            try {
                ...
                if (response.isSuccessful && response.body != null) {
                    ...
                    Handler(Looper.getMainLooper()).post {
                        if (msg.contains("验证成功") || msg.contains("正确")) {
                            Toast.makeText(this@RegisterCodeActivity, "验证码验证成功", Toast.LENGTH_SHORT).show()
                            // 跳转到设置密码页面
                            navigateToPasswordSetup(code)
                        } else {
                            Toast.makeText(this@RegisterCodeActivity, msg, Toast.LENGTH_SHORT).show()
                        }
                    }
                }
            } catch (e: IOException) {
                e.printStackTrace()
                Handler(Looper.getMainLooper()).post {
                    Toast.makeText(this@RegisterCodeActivity, "验证失败：网络错误", Toast.LENGTH_SHORT).show()
                }
            } catch (e: org.json.JSONException) {
                e.printStackTrace()
                Handler(Looper.getMainLooper()).post {
                    Toast.makeText(this@RegisterCodeActivity, "验证失败：数据解析错误", Toast.LENGTH_SHORT).show()
                }
            }
        }.start()
    }
    ...
    // 跳转到设置密码页面
    private fun navigateToPasswordSetup(code: String) {
